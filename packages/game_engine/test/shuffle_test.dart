...
void main() {
  group(
    ...
    () {
      test(
        'shuffles in place, keeping the same list instance',
        () {
      ...
      test(
        'produces a different order for a different seed',
        () {
      ...
      test(
        'does nothing to a single-element list',
        () {
          final items = [1];
      ...
      test(
        'uses a new Random when none is provided',
        () {
          final items = [1, 2, 3, 4, 5];
      ...
      test(
        'distributes each item roughly evenly across every position',
        () {
          const itemCount = 5;
          const trials = 20000;
